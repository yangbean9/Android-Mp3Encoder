...
#ifndef ANDROID_MP3ENCODER_COMMONTOOLS_H
#define ANDROID_MP3ENCODER_COMMONTOOLS_H
...
inline SInt16 TPMixSamples(SInt16 a, SInt16 b) {
    int tmp = a < 0 && b < 0 ? ((int) a + (int) b) - (((int) a * (int) b) / INT16_MIN) : (a > 0 &&
                                                                                          b > 0 ?
                                                                                          ((int) a +
                                                                                           (int) b) -
                                                                                          (((int) a *
...
inline void
convertShortArrayFromByteArray(byte *bytearray, int size, SInt16 *shortarray, float audioVolume) {
...
inline void convertAccompanySampleRateByAudioSampleRate(SInt16 *samples, SInt16 *samples_transfered,
                                                        int transfered_sample_count,
...
inline void
mixtureAccompanyAudio(SInt16 *accompanyData, SInt16 *audioData, int size, byte *targetArray) {
    byte *tmpbytearray = new byte[2];
    for (int i = 0; i < size; i++) {
...
inline void
mixtureAccompanyAudio(SInt16 *accompanyData, SInt16 *audioData, int size, short *targetArray) {
    for (int i = 0; i < size; i++) {
        SInt16 audio = audioData[i];
        SInt16 accompany = accompanyData[i];
...
#endif //ANDROID_MP3ENCODER_COMMONTOOLS_H
